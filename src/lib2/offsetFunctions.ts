...
/**
 * Data for the polygon offset operation.
 */
...
export function offsetToPathsOrPolyTree(polyTreeMode: boolean, nativeClipperLib: NativeClipperLibInstance, data: OffsetData): Paths | PolyTree | undefined {
  const filledData = {
    arcTolerance: 0.25,
    miterLimit: 2,
    ...data
  ...
  const addPathOrPaths = (inputDatas: OffsetInput[] | undefined) => {
  ...
  try {
    addPathOrPaths(data.offsetInputs);
    if (polyTreeMode) {
      return offset.executeToPaths(data.delta);
    }
    else {
      return offset.executeToPolyTree(data.delta);
    }
  }
  catch (err) {
    return undefined;
...
export function offsetToPaths(nativeClipperLib: NativeClipperLibInstance, data: OffsetData): Paths | undefined {
  return offsetToPathsOrPolyTree(false, nativeClipperLib, data) as (Paths | undefined);
}

export function offsetToPolyTree(nativeClipperLib: NativeClipperLibInstance, data: OffsetData): PolyTree | undefined {
  return offsetToPathsOrPolyTree(true, nativeClipperLib, data) as (PolyTree | undefined);
}
